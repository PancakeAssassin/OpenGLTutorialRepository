...
bool InitGL()
{
	...
	GLuint uLightPosition = glGetUniformLocation(ProgramID, "LightPosition");
...
void DrawScene()
{
	...
	//setup uniform value for light intensity, light position, and diffuse reflectivity
	GLuint uLightIntensity = glGetUniformLocation(ProgramID, "Ld");
	glUniform3f(uLightIntensity, LightIntensity.x, LightIntensity.y, LightIntensity.z);

	GLuint uDiffuse = glGetUniformLocation(ProgramID, "Kd");
	glUniform3f(uDiffuse, .94f, .84f, .74f);
